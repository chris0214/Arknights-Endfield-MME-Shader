...
public static class PmxReader
{
    public static string? ResolveTextureFilePath(string pmxPath, string? texturePath)
    {
        ...
        var normalized = texturePath.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);
        return Path.GetFullPath(Path.Combine(Path.GetDirectoryName(Path.GetFullPath(pmxPath))!, normalized));
    }
